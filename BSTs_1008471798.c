...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
     ...
     */
    double key_to_find = (10.0*bar)+index;
...
BST_Node *find_successor(BST_Node *right_child_node)
{
    ...
    //successor = smallest node in the right subtree of the node that u are trying to delete
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     * tree!
     */
    double key_to_check = (10.0*bar)+index;
    BST_Node *temp = NULL;
    if(root == NULL){
        return NULL;
    }
    if(root->key == key_to_check){
        ...
        if(root->right == NULL){
            ...
            //get successor for root
            temp = find_successor(root->right);
            //put data from successor to root
            root->bar = temp->bar;
            root->freq = temp->freq;
...
void delete_BST(BST_Node *root)
{
    ...
    //conduct post-order traversal
    delete_BST(root->left);
    delete_BST(root->right);
    //free nodes
    printf("Deleting Note -> Bar:Index (%d:%f), F=%f Hz\n", root->bar, root->index, root->freq);
    free(root);
}
...
void invert_tree(BST_Node *root, int BAR_MAX, int BAR_MIN){
    if(root == NULL){
        return;
    }
    else{
        //call to swap left and right subtrees
        invert_tree(root->left, BAR_MAX, BAR_MIN);
        invert_tree(root->right, BAR_MAX, BAR_MIN);
        ...
        //reverse values of index
        root->index = 1.0 - root->index;

        //reverse values of bar
        root->bar = (BAR_MAX + BAR_MIN) - root->bar; 
    }
}
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    invert_tree(root, find_MAX(root), find_MIN(root));
    return root;
}
...
int find_freq_index(BST_Node *note){
    for(int i = 0; i < 100; i++){
        if(note->freq == note_freq[i]){
            return i;
        }
    }
    return -1;
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     */
    BST_Node *traverse_node = root;
    if(traverse_node == NULL){
        return NULL;
    }
    else{
        BST_harmonize(traverse_node->left, semitones, time_shift);
        BST_harmonize(traverse_node->right, semitones, time_shift);

        BST_Node *temp = NULL;
        if(find_freq_index(traverse_node) + semitones >= 0 && find_freq_index(traverse_node) + semitones <= 99 && 
        traverse_node->index + time_shift >= 0 && traverse_node->index + time_shift < 1){
            temp = newBST_Node(find_freq_index(traverse_node)+semitones, traverse_node->bar, traverse_node->index+time_shift);
            if(temp == NULL){
                return root;
            }
